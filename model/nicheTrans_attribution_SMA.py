...
class NicheTrans(nn.Module):
    ...
    def forward(self, data, graph_meta=None, return_attention=False):
        b, n, _ = data.shape
        l = n - 1

        graph_context = self.local_graph_encoder.build_graph_context(data, graph_meta=graph_meta)
        ...
        f_omic = self.encoder(data.reshape(-1, self.source_length)).reshape(b, n, self.fea_size)
