...
class NicheTrans(StackedMoEModelMixin, nn.Module):
    ...
    def forward(self, source, source_neighbor, return_moe_info=False):
        ...
        neighbor_count = source_neighbor.size(1)
        spatial_tokens = torch.cat(
            [
                self.token_center,
                self.token_neigh_1.repeat(1, neighbor_count // 2, 1),
                self.token_neigh_2.repeat(1, neighbor_count // 2, 1),
            ],
            dim=1,
        )
        ...
        f_omic = self.encoder(omic_data).view(batch_size, -1, self.fea_size)
